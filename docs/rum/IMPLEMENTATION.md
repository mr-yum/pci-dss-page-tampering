...
**Verify a candidate sensor on real traffic before the full flip.** Every beacon carries `session.agentVersion`, and the collector dimensions its telemetry by it, so a staged rollout is observable without extra infrastructure: deploy the candidate to staging pages first (the inventory lane and staging alarms watch it under QA traffic), then serve it to a slice of production sessions and read the promotion gate straight off CloudWatch — zero `rum_beacons_rejected` attributed to the candidate version (schema rejects carry the claimed version; size and json rejects have nothing readable to attribute and land under `AgentVersion: unknown`, so watch that series for a step change during the rollout too; a hostile spray of fabricated versions collapses to `other` — a step change there is noise or abuse, never your rollout), `rum_agent_p95_task_ms` and `rum_agent_dropped` at parity with the current release, `rum_beacons_accepted_by_version` confirming the cohort share, and no alert-rate anomaly against baseline over the soak window. During the overlap both hashes are authorised (step 1 above), so the interlock holds for both versions; rolling back is redeploying the previous tag. A broken candidate fails noisy, never silent: garbage beacons meter as version-attributed rejects, phantom observations surface as alerts, and silence trips the volume anomaly alarm.
