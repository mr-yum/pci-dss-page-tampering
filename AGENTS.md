...
- **Repos with auto-review disabled**: CodeRabbit posts a "Review skipped" notice instead of reviewing, and pushes never trigger rounds automatically. Comment `@coderabbitai review` on the PR to trigger a round — after every push, since nothing runs on its own — then wait for it like any other round. Do not assume this per repository: the script-inventory repo used to behave this way, but on 2026-09-11 it auto-reviewed a new PR on open and then auto-paused ("Reviews paused … branch is under active development"), and a `@coderabbitai review` posted after that round answered `Already reviewed the last commit` and ran nothing. Read the first CodeRabbit comment on the PR to learn which mode you are in; after a pause, `@coderabbitai review` reviews only new commits and `@coderabbitai full review` re-reviews everything.
