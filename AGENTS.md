...
```bash
gh api --paginate --slurp repos/<org>/<repo>/pulls/<n>/reviews | jq 'add | [.[] | select(.user.login|test("coderabbit"))] | last | {submitted_at, marker: (.body | split("\n")[0])}'
```
