...
- **CspDirectiveMatcher** (`src/types/matcher/csp-directive-matcher.ts`) - Matches one Content-Security-Policy directive by its **set** of source expressions rather than the literal header text. Order-insensitive, tolerant of removals (a policy allowing fewer origins is strictly safer), strict about additions — and the failure reason names the offending sources. Use for CSP directives instead of an anchored `contentMatcher`: a literal pattern mints a fresh near-duplicate alternative every time the application reorders or drops a source, which is how entries accumulate a dozen-plus alternatives. Source expressions are compared exactly and wildcards are **not** expanded (`https://*.example.com` and `https://a.example.com` are different assertions); directive names are case-insensitive per the CSP spec. Emitted automatically by the inventory workflow for `content-security-policy` and `content-security-policy-report-only` (`src/utils/header.ts`).
