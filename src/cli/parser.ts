...
function normalizeKey(key: string): keyof RawCliArgs | null {
  ...
  const validKeys: (keyof RawCliArgs)[] = ['mode', 'target', 'repo', 'gitToken', 'slackToken', 'inventoryBranch', 'detectionBranch', 'gitUserName', 'gitUserEmail', 'totpSeed', 'reportDir', 'rumQueueUrl', 'help']
