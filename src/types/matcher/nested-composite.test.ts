...
describe('Nested Composite Matchers', () => {
  const mockMatchable = (name: string, content: string): Matchable => ({
    name,
    content,
    hash: undefined,
  })
  ...
  describe('T054: Nested OR containing AND (first AND group succeeds)', () => {
    it('should authorize when first AND group matches all children', () => {
      ...
      const andGroup1 = new AndMatcher(
        [new ContentMatcher(/foo/), new ContentMatcher(/bar/)],
        authInfo('AND group 1: requires foo and bar'),
      )

      const andGroup2 = new AndMatcher(
        [new ContentMatcher(/baz/), new ContentMatcher(/qux/)],
        authInfo('AND group 2: requires baz and qux'),
      )
      ...
      const resource = mockMatchable('test-resource', 'This content has foo and bar in it')
  ...
  describe('T055: Nested OR containing AND (second AND group succeeds)', () => {
    it('should authorize when second AND group matches all children', () => {
      ...
      const resource = mockMatchable('test-resource', 'This content has baz and qux in it')
  ...
  describe('T056: Nested OR containing AND (partial match of both groups, neither complete)', () => {
    it('should deny when no AND group fully matches', () => {
      ...
      const resource = mockMatchable('test-resource', 'This content has foo and baz but not complete groups')
  ...
  describe('T057: Nested AND containing OR', () => {
    it('should authorize only when all OR children succeed', () => {
      ...
      const resource = mockMatchable('test-resource', 'This content has foo and baz')
    ...
    it('should deny when first OR group fails', () => {
      ...
      const resource = mockMatchable('test-resource', 'This content only has baz')
  ...
  describe('T058: Deeply nested composites (5+ levels)', () => {
    it('should handle 5 levels of nesting correctly', () => {
      ...
      const resource = mockMatchable('test-resource', 'Content with alpha beta delta')
    ...
    it('should handle short-circuit path through 5 levels', () => {
      ...
      const resource = mockMatchable('test-resource', 'Content with epsilon only')
  ...
  describe('T059: Metadata path collection through nested composites', () => {
    it('should collect metadata from all evaluated composite levels', () => {
      const andGroup = new AndMatcher(
        [new ContentMatcher(/foo/), new ContentMatcher(/bar/)],
        authInfo('AND: foo and bar required'),
      )
      ...
      const resource = mockMatchable('test-resource', 'Content with foo bar qux')
    ...
    it('should include partial path on authorization failure', () => {
      ...
      const resource = mockMatchable('test-resource', 'Content with foo bar and qux')
